...
{{--            {!! Form::text('fact_lcal_id',auth()->getUser()->Local->lcal_id, ["class" => "form-control d-none", "placeholder"=>""]) !!}--}}
            ...
            <h4>Local / Receptor de factura</h4>

            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        {{Form::label('fact_lcal_id','Local Asignado',['class'=>'col-sm-4 control-label'])}}
                        {!! Form::select('fact_lcal_id', $locales ,  null , ["class"=>"form-control select2-control filter-control",
                                    "placeholder" => "Local", "style" => "width: 100%" ])!!}
                    </div>
                </div>

                <div class="col-sm-6 text-center" id="ph-tabla-comprobantes">

                    <p>Comprobantes a facturar</p>
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                    <div class="form-group">
                        ...
                        {!! Form::text('fact_nombre_receptor', null ,["class"=>"form-control", "placeholder" => "", "readonly"=>true]);!!}
                    </div>
                </div>
                ...
                <div class="col-sm-3">
                    <div class="form-group">
                        ...
                        {!! Form::text('fact_rfc_receptor',null,["class"=>"form-control", "placeholder" => "", "readonly"=>true]);!!}
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        jNuevaFactura = {
          ...
          get_local_data_url:  '{{url('factura/get-local-data')}}',
          modal_size:'modal-md',
          productos : {!!  $productos !!},
          unidades : {!!  $unidades !!} ,
          conceptos: [],
          ...
          comprobantes_factura : [],
           ...
           init: function(){
              ...
              $('#fact_lcal_id').on('change', function(){

                  let lcal_id = $(this).val();

                  if(lcal_id > 0){
                      $this.getLocalData(lcal_id);
                  }
            ...
            getLocalData: function(lcal_id){
                let $this = this;
                $.ajax({
                    url: $this.get_local_data_url + '/' + lcal_id,
                    method: 'POST',
                    beforeSend:function(){
                        $('#ph-tabla-comprobantes').html("<span> <i class='fa fa-gear fa-spin'></i> Cargando información</span>");
                    ...
                    success: function (res) {
                        ...
                        $('#ph-tabla-comprobantes').html(res.vista_comprobantes);

                        $('#fact_rfc_receptor').val(res.local.lcal_rfc);
                        $('#fact_nombre_receptor').val(res.local.lcal_razon_social);

                        setTimeout(function() { $this.setCheckComprobantesListeners(); }, 500);
            ...
            setCheckComprobantesListeners: function(){
              ...
              $('.checkbox-comprobante').off().on('change',function(){
                  if(this.checked){
                      ...
                      key = $this.comprobantes_factura.find(function(el){
                         return el.cpag_id ==  cpag_id;
                      });

                      $this.comprobantes_factura.splice(key+1,1);
                      $this.createConceptoFromComprobantes();
